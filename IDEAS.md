...
A key is a string, typically written as a symbol. A value is any expression. The pair is separated by a colon.
...
A hash is a collection of key-value pairs surround by curly braces. Each pair is separated by a comma.
...
imported using the `with` keyword followed by a hash specifying the type. The only restriction is the type must be registered first.
